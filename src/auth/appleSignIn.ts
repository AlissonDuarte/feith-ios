 ...
 *    Android), entao e uma pergunta assincrona de runtime — nao da para
 *    exportar uma constante como `googleSignInAvailable`.
 ...
 *
 * Nao funciona no Expo Go — o modulo e nativo.
 */
 ...
 *
 * @returns o JWT (e o perfil, se for a primeira entrada), ou `null` se o
 *   usuario cancelou.
 */
export async function signInWithApple(): Promise<AppleCredential | null> {
  try {
    ...
    if (!credential.identityToken) {
      throw new AppleSignInError('A Apple não devolveu um token de identidade.');
    }
    ...
    throw new AppleSignInError('Não foi possível entrar com a Apple.');
  }
}
